...
% find attitude error (gmo-pointing frame) 
RcN = Rc2dcm(0); 
N_omega_RcN = RcNomega(0); 
[sigma_BR, B_omega_BR] = tracking_error(0, sigma_BN0, B_omega_BN0, RcN, N_omega_RcN); 
...
for k = 1:length(t)-1
    ...
    % compute control 
    RcN = Rc2dcm(t(k));
    N_omega_RcN = RcNomega(t(k)); 
    [sigma_BR, B_omega_BR] = tracking_error(t(k), X(1:3, k), X(4:6, k), RcN, N_omega_RcN); 
    if norm(sigma_BR) >= 1
