...
RsN = inertial2sunpointing; 
N_omega_RsN = zeros(3, 1); 
[sigma_BR, B_omega_BR] = tracking_error(0, sigma_BN0, B_omega_BN0, RsN, N_omega_RsN); 
...
for k = 1:length(t)-1
    ...
    % compute control 
    [sigma_BR, B_omega_BR] = tracking_error(t(k), X(1:3, k), X(4:6, k), RsN, N_omega_RsN); 
    if norm(sigma_BR) >= 1
